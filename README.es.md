...
| Superficie | Estado |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.5-alpha.1` (tag de GitHub; verificado el 2026-09-09: cadena completa de puertas + smoke de instalación de perfil). Pin npm `0.1.2-rc.1`, peers `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0`. |
...
| Eventos `session/pin` | Con compuerta previa: nunca se escriben en hosts cuyo vocabulario de eventos no conoce el tipo y cuyo append dejó caer el marcador `ignorable` (`0.1.2-rc.1`); la proyección degrada a la caché de settings 0.1.2-rc.1 (adaptado el 2026-09-04): el sobre de sesión conserva su campo ignorable solo para compatibilidad de lectura de logs almacenados - Session.append aún no puede estamparlo, por lo que el comportamiento de la puerta no cambia. |
