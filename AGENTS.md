...
- `src/index.ts` — function-plugin contract (`name`/`inject`/`Config`/`apply`; NO default export — the Loader unwraps `exports.default ?? exports`). Injects `settings` only. On the `0.1.7` settings contract the plugin's durable settings surface IS its own live Config: every former `session-pin` namespace field is a `.volatile()` field on `Config` (pin lists, color maps, organizer state, host policy), the profile entry id (`SETTINGS_ENTRY_ID`, the `cordis.patch.yml` row id) is the form namespace, and `apply` claims the generated-form presentation policy plus, under `enableLogBacking`, the log-backed projection reader. `enableLogBacking` alone stays ordinary Config — host-only, never a field of the old namespace.
...
- `src/reorder-pump.ts` — the re-assertion guard (issue #4): one pass at a time (a list change landing during a pass collapses into a single re-check), moves applied sequentially against the live order, and a plan issued at most once per observed order — so the list-change echo of this client's own confirmed move cannot re-enter the reorder path. `connection/reset` re-arms it.
- `src/client.ts` — browser half: store + controller + overlay + row-slot glue + slot contributions. Reads the host half's live Config form through `ctx.configForms.get(entryId)` (the `settingsScope` service is gone; `ConfigForm` is its documented successor), the `SessionId`/`WorkspaceId` brands from `@deepseek-ai/dsh-client-connection/client` (the removed `dsh-client-runtime` package must never reappear in this repo's imports or `dsh.client.inject`), and `connection` for the optional `session.setPinned` channel.
