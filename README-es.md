...
| Superficie | Estado |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.6-alpha.2` (tag de GitHub; verificado el 2026-09-18: typecheck de doble regla + comprobaciones estáticas de costuras; la ronda de navegador queda a cargo del mantenedor). Pin npm `0.1.6-alpha.2`, peers `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0`. |
...
- **Lectura de proyección respaldada por log** — `enableLogBacking` (Config del host, por defecto desactivado en modo fail-closed) monta un lector que pliega eventos `session/pin` en vivo al conjunto canónico y refleja `pinned`/`colors` en el namespace de settings. El esquema, el pliegue puro (`foldPinEvents`) y la costura de anexado con compuerta previa (`PinLogAppender`) viven en `src/pin-log.ts`: el vocabulario de eventos en tiempo de ejecución es la única señal de la puerta, decidida ANTES del primer anexado (el append de la línea alpha ya no puede estampar `ignorable`, así que la sonda del marcador desapareció), de modo que los hosts que no pueden transportar el evento con seguridad — un vocabulario que no conoce el tipo falla cerrado al leer — nunca reciben uno; el almacén settings/localStorage sigue siendo la ruta de compatibilidad y degradación.
