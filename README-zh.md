...
| 维度 | 状态 |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.7-rc.2`（GitHub tag，2026-09-22 已核验：双尺子 typecheck + 单元/组合测试套件 + 静态接缝检查；浏览器人工轮待维护者）。npm 依赖钉号 `0.1.7-rc.2`，peers `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0 || >=0.1.7-0 <0.2.0`。 |
| Node | `>= 22`（开发环境下限） |
