...
| Superfície | Status |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.5-alpha.1` (tag do GitHub; verificado em 2026-09-09: cadeia completa de portas + smoke de instalação de perfil). Pin npm `0.1.2-rc.1`, peers `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0`. |
...
| Eventos `session/pin` | Com portão prévio: nunca escritos em hosts cujo vocabulário de eventos não conhece o tipo e cujo append deixou cair o marcador `ignorable` (`0.1.2-rc.1`); a projeção degrada para o cache de settings 0.1.2-rc.1 (adaptado em 2026-09-04): o envelope de sessão mantém seu campo ignorable apenas para compatibilidade de leitura de logs armazenados - o Session.append ainda não consegue estampá-lo, então o comportamento da porta não muda. |
